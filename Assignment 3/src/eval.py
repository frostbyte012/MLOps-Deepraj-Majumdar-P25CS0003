...
def evaluate(model_path: str, output_path: str = "results/eval_results.json") -> dict:
    ...
    # If loading from Hub, verify model weights exist first
    if "/" in model_path and not os.path.isdir(model_path):
        from huggingface_hub import HfApi
        try:
            api = HfApi()
            files = list(api.list_repo_files(model_path))
            print(f"Files found on Hub: {files}")
            ...
            if not has_weights:
                raise OSError(
                    ...
                    "Make sure training completed and --push_to_hub ran successfully."
                )
        except Exception as e:
            raise OSError(f"Could not verify Hub repo {model_path}: {e}")
