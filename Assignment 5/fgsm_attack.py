...
@torch.enable_grad()  # <-- FIX: Force gradients to stay on!
def fgsm_attack_scratch(model, imgs, labels, epsilon, device):
    """Pure PyTorch FGSM implementation."""
    model.eval()  # keep BN/dropout stable but we need grads on input
    imgs_adv = imgs.clone().detach().to(device).requires_grad_(True)
    criterion = nn.CrossEntropyLoss()
    ...
    imgs_adv = (imgs_adv + perturbation).detach()
    imgs_adv = torch.clamp(imgs_adv, -3, 3)
    return imgs_adv
...
def save_fgsm_comparison(imgs_clean, imgs_adv_scratch, imgs_adv_art,
    ...
    # <-- FIX: Removed the wrapper torch.no_grad() here
...
def main():
    ...
    if run:
        ...
        _, (_, imgs_adv_art_sample, _) = evaluate_fgsm_art(
            art_cls, [(imgs_batch, labels_batch)], 0.03)
        for i in range(10):
